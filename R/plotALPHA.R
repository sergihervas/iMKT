...
plotALPHA <- function(x, y, df, xlow, xhigh) {
  ...
  #plotting
  p <- ggplot(x, aes_string(x="daf", y="alpha")) + geom_point(size=3) +
    #function curve
    ...
    geom_rect(data=data.frame(xmin=-Inf, xmax=Inf, ymin=CI_low ,ymax=CI_high), aes_string(xmin="xmin", xmax="xmax", ymin="ymin", ymax="ymax"), fill="gray30", alpha=0.4, inherit.aes=F) +
