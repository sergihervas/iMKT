...
plotDAF <- function(x) {
  ...
  p1 <- ggplot() +
    geom_point(data=x, aes_string(x="daf", y="pS", color="p0"), size=3) + 
    geom_point(data=x, aes_string(x="daf", y="pN", color="pi"), size=3) +
    theme_classic() +
  ...
  p2 <- ggplot() +
    geom_point(data=x, aes_string(x="daf", y="S", color="p0"), size=3) + 
    geom_point(data=x, aes_string(x="daf", y="N", color="pi"), size=3) +
    theme_classic() +
    theme(axis.text=element_text(size=rel(1.5))) +
    theme(axis.title=element_text(size=rel(1.5))) +
